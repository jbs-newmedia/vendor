...
foreach (glob(\osWFrame\Core\Settings::getStringVar('settings_framepath') . 'oswcore' . \DIRECTORY_SEPARATOR . 'oswvendor' . \DIRECTORY_SEPARATOR . 'namespaces' . \DIRECTORY_SEPARATOR . '*') as $vendor) {
    ...
    if ($products !== []) {
        ...
        foreach (glob($vendor . \DIRECTORY_SEPARATOR . '*') as $product) {
            ...
            if ($vendor_versions !== []) {
                if (!isset($vendor_settings[$vendor_name . '_' . $product_name])) {
...
foreach (glob(\osWFrame\Core\Settings::getStringVar('settings_framepath') . 'oswproject' . \DIRECTORY_SEPARATOR . 'oswvendor' . \DIRECTORY_SEPARATOR . 'namespaces' . \DIRECTORY_SEPARATOR . '*') as $vendor) {
    $vendor_name = strtolower(str_replace(['.'], ['_'], basename($vendor)));
    $products = glob($vendor . \DIRECTORY_SEPARATOR . 'version-*');
    if ($products !== []) {
        if (!isset($vendor_settings[$vendor_name])) {
